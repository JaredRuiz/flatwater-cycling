...
MapLine.prototype.generateDoublePoints = function(min, max) {
  ...
  for (var j = 0, max = this.mainLine.length; j < max; j++) {
    if (j < this.mainLine.length-1) {
      ...
      // TODO give makeOnePointTwo an optional function arg??
...
var getLineBounds = function(lineArr) {
  var tempBounds = { 
    north: [],
    south: [],
    east: [],
  ...
  lineArr.forEach(function(line) {
    tempBounds.north.push(line.north());
    tempBounds.south.push(line.south());
    tempBounds.east.push(line.east());
    tempBounds.west.push(line.west());
  });

  var finalBounds = {
    north: Math.max.apply(Math, tempBounds.north),
    south: Math.min.apply(Math, tempBounds.south),
    east: Math.max.apply(Math, tempBounds.east),
    west: Math.min.apply(Math, tempBounds.west)
  };
